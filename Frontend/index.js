...
async function ObtenerUsuario(accessToken) {
    try {
        const response = await fetch('http://' + urlActual + ':8082/api/auth/user', {
            method: 'GET',
            headers: {
...
async function logoutFunction() {
    try {
        ...
        const response = await fetch('http://' + urlActual + ':8082/api/auth/logout', {
            method: 'GET',
            headers: {
                Authorization: "Bearer " + accessToken
            }
...
async function InicializarMapa() {
    ...
    const accessToken = sessionStorage.getItem('accessToken');
    try {
        const response = await fetch('http://' + urlActual + ':8082/api/RecogerMunicipios', {
            method: 'GET',
            headers: {
                Authorization: "Bearer " + accessToken
            }
...
function MostrarTooltip() {
    var lugares = [
        ...
        { "nombre": "Usurbil", "region": 'basque_country', "zone": 'coast_zone', "location": 'usurbil', "id": '5' }
    ];
...
async function imprimirFechas() {
    var fecha1 = document.getElementById("fecha1").value;
    var fecha2 = document.getElementById("fecha2").value;
    var nombreCapital = document.getElementById("nombrecapital").innerText;

    // Comprobar si todas las fechas y el nombre de la capital han sido seleccionados
    if (fecha1 === "" || fecha2 === "") {
        alert("Por favor, rellena todas las fechas y selecciona un nombre de capital.");
        return;
    }
    ...
    // Comprobar si la fecha 1 es menor o igual que la fecha 2
    if (date1 > date2) {
        alert("La primera fecha debe ser anterior o igual a la segunda fecha.");
        return;
    }
    ...
    console.log("Nombre de capital:", nombreCapital);


    try {
        const response = await fetch(`http://localhost:8082/api/consultar-historico?fecha_inicio=${fecha1}&fecha_fin=${fecha2}&nombre_capital=${nombreCapital}`, {
            method: 'GET',
            headers: {}
        });
        ...
        if (response.ok) {
            ...
            var contenedor = document.querySelector('.graficocontainer');
            contenedor.innerHTML = '';
            // Crear un nuevo canvas
            var nuevoCanvas = document.createElement('canvas');
            nuevoCanvas.id = 'miGraficoNuevo'; // Asigna un id único
            contenedor.appendChild(nuevoCanvas);

            // Obtener el contexto del lienzo
            var ctx = nuevoCanvas.getContext('2d');

            // Extraer los datos del resultado de la API
            ...
            var data = result.map(item => item.avg_temperatura);

            // Datos del gráfico
            var chartData = {
                labels: labels,
                datasets: [{
                    label: 'Temperatura Promedio ' + nombreCapital ,
                    data: data,
...
function seleccionarFecha(valor) {
    document.getElementById("nombrecapital").innerText = valor;
}